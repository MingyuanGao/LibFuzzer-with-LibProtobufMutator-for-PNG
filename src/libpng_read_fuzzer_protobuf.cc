...
/* NOTE Use the correct macro, i.e., DEFINE_PROTO_FUZZER or
 * DEFINE_BINARY_PROTO_FUZZER
 *
 * For example, to use the PNG seeds in "seeds-protobuf" dir, use the
 * "DEFINE_BINARY_PROTO_FUZZER" macro.
 */
DEFINE_BINARY_PROTO_FUZZER(const PngProto &png_proto) {
  auto s = Proto2PNG(png_proto);

  // std::size_t png_proto_hash = std::hash<std::string>{}(
  // png_proto.SerializeAsString() ); std::string dump_path =
  // std::to_string(png_proto_hash) + ".png"; std::ofstream of(dump_path);
...
std::string Proto2PNG(const PngProto &png_proto) {
  ...
  std::cout << "w=" << ihdr.width() << ", h=" << ihdr.height() << std::endl;
  // Avoid large images.
  ...
  for (size_t i = 0, n = png_proto.chunks_size(); i < n; i++) {
    ...
    if (chunk.has_plte()) {
      ...
      WriteChunk(all, "IDAT", chunk.idat().data(),false); // TODO: true or false?
      ...
      if (other_chunk.has_known_type()) {
       	uint32_t known_type_int = other_chunk.known_type(); 
        /* NOTE the function here! LibPNG uses big-endian numbers! */
        type_int = __builtin_bswap32(known_type_int); 
...
//	memcpy(type, known_chunks[chunk_idx], 4);
        ...
        /* NOTE the function here! LibPNG uses big-endian numbers! */
        type_int = __builtin_bswap32(unknown_type_int); 
      } else {
        continue;
      }
