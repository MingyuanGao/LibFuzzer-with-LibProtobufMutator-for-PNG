...
int png2protobuf (const char* in_file, const char* out_file) {
	...
	for (;;) {
		...
		if (buf[4] == 'I' && buf[5] == 'H' && buf[6] == 'D' && buf[7] == 'R' ) { // IHDR
			uint32_t width; /* must be 32 bits! */
   			GETBREAK; width  = c; width <<= 8;
   			GETBREAK; width += c; width <<= 8;
   			GETBREAK; width += c; width <<= 8;
   			GETBREAK; width += c;

			uint32_t height; /* must be 32 bits! */
   			GETBREAK; height  = c; height <<= 8;
   			GETBREAK; height += c; height <<= 8;
   			GETBREAK; height += c; height <<= 8;
   			GETBREAK; height += c;
			
			uint32_t other1; /* must be 32 bits! */
   			GETBREAK; other1  = c; other1 <<= 8;
   			GETBREAK; other1 += c; other1 <<= 8;
   			GETBREAK; other1 += c; other1 <<= 8;
   			GETBREAK; other1 += c;
		...
		if (buf[4] == 'P' && buf[5] == 'L' && buf[6] == 'T' && buf[7] == 'E' ) {
			...
			std::cout << "\ndata:\n  ";
			for (const auto e : plte_data) printf("%x ",e);
			printf("\n");
		...
		if (buf[4] == 'I' && buf[5] == 'D' && buf[6] == 'A' && buf[7] == 'T') {
			...
			std::cout << "\ndata:\n  ";
			for (const auto e : idat_data ) printf("%x ",e);
			printf("\n");	
		...
		if (buf[4] == 'i' && buf[5] == 'C' && buf[6] == 'C' && buf[7] == 'P') {
			...
			std::cout << "\ndata:\n  ";
			for (const auto e : iccp_data) printf("%x ",e);
			printf("\n");	
		...
		if ( // known chunk types  
			...
			std::cout << "\ndata:\n  ";
			for (const auto e : other_data) printf("%x ",e);
			printf("\n");	
	...
	// Write to disk
	std::fstream output(out_file, std::ios::out | std::ios::trunc | std::ios::binary);
 	...
 	if(png_proto.has_ihdr()) {
 	 ...
 	 for (size_t i = 0, n = png_proto.chunks_size(); i < n; i++) {
		 ...
		 if (chunk.has_plte()) {
			 ...
			 type_int = __builtin_bswap32(type_int);  // NOTE this function here!
